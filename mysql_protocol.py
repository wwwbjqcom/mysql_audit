...
class mysql_packet(object):
    ...
    def COM_QUERY(self):
        ...
        """
        return self.data[self.offset:],['OK_Packet','ERR_Packet','Text_Resultest']
    ...
    def COM_INIT_DB(self):
        ...
        server return:
            ...
            ERR_Packet on error
        """
        return self.data[self.offset:],['OK_Packet','ERR_Packet']
    ...
    def COM_SET_OPTION(self):
        ...
        """
        return struct.unpack('<H',self.data[self.offset:self.offset+2]),['OK_Packet','ERR_Packet']
    ...
    def COM_STMT_FETCH(self):
        ...
        return struct.unpack('<I', self.data[self.offset:self.offset + 4])[0],['ERR_Packet','Text_Resultest']
    ...
    def COM_STMT_RESET(self):
        ...
        return struct.unpack('<I', self.data[self.offset:self.offset + 4])[0],['OK_Packet','ERR_Packet']
    ...
    def Connection_Packets(self):
        ...
        return 'Connection_Packets',['Handshake_Packet']
    ...
    def Handshake_Packet(self):
        ...
        return 'Handshake_Packet','create connection'
    ...
    def ERR_Packet(self):
        ...
        """
        return 'ERR_Packet',self.data[self.offset+2:]
    ...
    def check_server_response(self,packet_header):
        ...
        elif packet_header in (0x09,0x10):
            return self.Handshake_Packet()
        else:
            return self.Text_Resultest()

    def Unpacking(self,data,srchost,srcport,dsthost,dstport):
        """
        unpack packet
        :return:
        """
        ...
        self.offset = 0
        self.packet_palyload = struct.unpack('B', self.data[2])[0] << 16 | struct.unpack('B', self.data[1])[0] << 8 | \
                          struct.unpack('B', self.data[0])[0]
        self.offset += 3
        packet_seq_id = struct.unpack('B', self.data[self.offset])[0]
        self.offset += 1
        packet_header = struct.unpack('B', self.data[self.offset])[0]
        self.offset += 1
        if self._type == 'src':
            if srchost == self._ip:
                '''client packet'''
                session = str([srchost,srcport,dsthost,dstport])
                if packet_header in self.client_packet_type:
                    client_packet_text,response_type = self.client_packet_type[packet_header]()
            else:
                '''server response'''
                session = str([dsthost, dstport, srchost, srcport])
                packet_response,response_status = self.check_server_response(packet_header)
        ...
        return session,packet_response,client_packet_text,packet_header,packet_seq_id,response_type,response_status